...
function pg__data_init
{
    ...
    is_pg_cluster_exists || (
        ...
        # copy configuration to $PGDATA directory
        ...
        #su postgres -c "/usr/bin/pg_conftool set hba_file $PGDATA/pg_hba.conf"
        #su postgres -c "/usr/bin/pg_conftool set ident_file $PGDATA/pg_ident.conf'"
...
function set_pg_config()
{
    ...
    echo "host all all 10.255.0.0/16 trust" >> $_CON_FILE # swarm default network
